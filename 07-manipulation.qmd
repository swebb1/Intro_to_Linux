...
In this section we are going to use some common Unix tools to manipulate a genomic annotation file `GRCm39_ensembl_v110.txt`.
...
-   Output a UCSC style [BED file](https://en.wikipedia.org/wiki/BED_(file_format)), a standard genomic annotation format with six columns
...
awk '{ OFS="\t" }{ if($4==1){ print "chr"$1,$2,$3,$5,0,"+" }else{ print "chr"$1,$2,$3,$5,0,"-" }}' GRCm39_ensembl_v110.pc.txt > GRCm39_ensembl_v110.pc.edit.bed
...
To sort the file by chromosome and then by start position we can use the `-k` flag to set sorting keys. The first k, k1 uses column 1 with the `-V` flag. This is a "version" sort which is alphanumeric and will put chr1 first. The second key, k2, uses column 2 with `-n` flag. This is a numerical sort and will place lower start positions first.
